...
def featureEngineering(data_dict,features):
    for name in data_dict:
        try:
            numerator = data_dict[name]["from_poi_to_this_person"] + 
            			data_dict[name]["from_this_person_to_poi"] + 
            			data_dict[name]["shared_receipt_with_poi"]
            denominator = data_dict[name]['from_messages'] + 
...
def evaluation_loop(features, labels, num_iters=1000, test_size=0.3):
    ...
    evaluation_matrix = [[] for n in range(3)]
...
if __name__ == '__main__':
	...
	features_finance    = ["bonus", "deferral_payments", "deferred_income", "director_fees", "exercised_stock_options", "expenses", "loan_advances", "long_term_incentive", "other", "restricted_stock", "restricted_stock_deferred", "salary", "total_payments", "total_stock_value"]
	...
	ordered_list, summary = evaluation_loop(features, labels, num_iters=100, test_size=0.3)
